(async () => {
    ...
    const loadHome = async () => {
        ...
        if (!res || !res.data) {
            document.body.innerHTML = '<div style="text-align:center; padding:50px;">Gagal memuat berita.</div>';
            return;
        }

        const articlesHtml = res.data.map(news => {
            // Setup variabel dinamis
            const postUrl = `/${news.slug}`; 
            const title = news.title;
            const imageUrl = news.image_url || 'https://via.placeholder.com/1200x720';
            const category = news.category || 'Berita'; // Sesuaikan dengan properti API Anda
            const date = formatSimpleDate(news.created_at || news.updated_at); // Fungsi pemformat waktu
                ...
                <article class="dynamic-content-template post type-post status-publish format-standard has-post-thumbnail hentry">
                    <div class="gb-element-1ac029cb">
                        <div class="gb-element-96976bd5">
                            <h2 class="gb-text gb-text-9cbdff50 limit_mob">
                                <a href="${postUrl}" data-link>${title}</a>
                            </h2>

                            <div class="gb-element-568e7728">
                                <p class="gb-text gb-text-c8e5aff6">
                                    <a href="/category/${category.toLowerCase()}" rel="tag">${category}</a>
                                </p>

                                <p class="gb-text-bc7d981b">
                                    <span class="gb-shape">
                        ...
                        <div class="gb-element-edb99aad round">
                            <figure style="aspect-ratio:4/3" class="wp-block-post-featured-image">
                                <a href="${postUrl}" target="_self" data-link>
                                    <img width="1200" height="720" src="${imageUrl}" class="attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="${title}" style="width:100%;height:100%;object-fit:cover;" decoding="async" loading="lazy">
                                </a>
                            </figure>
                        </div>
                    </div>
                        ...
                        <div class="inside-right-sidebar">
                            ...
                            <div class="gb-element-db4ccbfb">
                                <div class="gb-element-0852c301">
                                    <div>
                                        <div id="sidebar-ad-slot" style="width:100%; min-height:250px; background:#f9f9f9; text-align:center; display:flex; align-items:center; justify-content:center;">
                                    ...
                                    <div>
                                        ...
                                        <div class="order_by_views">
                                            <div class="gb-looper-a17ef4e9 list_custom">
                                                <!-- Jika Anda punya data popular post dari API, Anda bisa meloopingnya di sini. Sementara ini adalah bentuk statis -->
                                                <div class="gb-loop-item gb-loop-item-62960bc0">
                                                    <div>
                                                        <h2 class="gb-text gb-text-69ef7f63"><a href="#">Contoh Judul Artikel Populer 1</a></h2>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    const loadDetail = async (slug, type = "news") => {
        ...
        const detailHtml = `
            <article id="post-single" class="post type-post status-publish format-standard has-post-thumbnail hentry">
                <div class="inside-article">
                    
                    <!-- Tempat Iklan Atas Judul (Opsional) -->
                    <div class="ad-container-top" style="text-align:center; margin-bottom: 20px;">
                        <div id="ads-728x90" style="display:inline-block; width:100%; min-height:90px; margin: 0 auto; background:#f9f9f9;"></div>
                    </div>

                    <!-- Header Artikel GeneratePress -->
                    <header class="entry-header">
                        <h1 class="entry-title" itemprop="headline">${cleanTitle}</h1>
                        <div class="entry-meta">
                            <span class="posted-on">
                                <time class="entry-date published" datetime="${pubDateUTC}">${formatSimpleDate(news.created_at || news.updated_at)}</time>
                            </span>
                            <span class="byline">
                                <span class="author vcard" itemprop="author">
                                    <span class="url fn n">Admin</span>
                                </span>
                            </span>
                        </div>
                    </header>

                    <!-- Featured Image GeneratePress -->
                    <div class="featured-image page-header-image-single">
                        <img width="1200" height="720" src="${detailImage}" class="attachment-full size-full wp-post-image" alt="${cleanTitle}" itemprop="image" decoding="async" loading="lazy" style="width:100%; height:auto;">
                    </div>

                    <!-- Konten Artikel GeneratePress -->
